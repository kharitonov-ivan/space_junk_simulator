...
class space_simulator:
    def __init__(self):
        ...
        libfile = ('/python_package.so')
        self.solver = ctypes.CDLL(cur_path + libfile)
        self.solver.main_.argtypes = [numpy.ctypeslib.ndpointer(dtype=numpy.float64, flags="C_CONTIGUOUS"), # x
        ...
        self.solver.main_.restype = None
        print("Ok!")

    def run(self, x, y, z, vx, vy, vz, vzsteps=10, timestep = 1.0):
        x_res, y_res, z_res, vx_res, vy_res, vz_res = x, y, z, vx, vy, vz
        self.solver.main_( x, y, z, vx, vy, vz, x_res, y_res, z_res, vx_res, vy_res, vz_res, vzsteps, timestep)
        print("run!")
