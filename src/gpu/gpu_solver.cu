...
__host__ __device__
double GpuGetAirDensity(double height) {
...
void GetAirDragAcc(double x, double y, double z,
  ...
  double air_drag = air_drag_coeff * GpuGetAirDensity(r) * v_norm;
...
void GetAccel(double x, double y, double z,
  ...
  GetSimpleGravityForceAcc(x, y, z, vx, vy, vz, ax, ay, az);
//  GetHeterogeneousGravityAcc(x, y, z, vx, vy, vz, ax, ay, az);
//  GetAirDragAcc(x, y, z, vx, vy, vz, ax, ay, az);
};
...
struct next_pos
  ...
  next_pos(double t, double dt) : t(t), dt(dt) {};
